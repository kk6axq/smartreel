...
# Mock parts catalog for the Load-screen QR flow (generated by
# scripts/gen_mock_qr.py). Copied to the SD root as parts.json if present.
if [ -f "$REPO/sd-assets/parts.json" ]; then
    cp "$REPO/sd-assets/parts.json" "$SD/parts.json"
    echo "  -> parts.json  (mock QR catalog)"
fi
sync
